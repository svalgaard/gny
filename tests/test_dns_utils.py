...
from gny.dns_utils import get_a_records, get_ptr_record, get_ptr_records
...
class TestGetPtrRecords:
    async def test_returns_single_record(self):
        mock_answer = MagicMock()
        mock_answer.__str__ = lambda self: "host.example.com."

        with patch("gny.dns_utils.dns.resolver.resolve", return_value=[mock_answer]):
            result = await get_ptr_records("1.2.3.4")

        assert result == ["host.example.com"]

    async def test_returns_multiple_records(self):
        mock_a = MagicMock()
    ...
    async def test_results_are_lowercased(self):
        mock_answer = MagicMock()
        mock_answer.__str__ = lambda self: "Host.Example.COM."
